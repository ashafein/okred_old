...
<?php
include("var.php");
echo "<head><title>Администрирование - Резюме дня: $sitename</title>";
echo "<META NAME=ROBOTS CONTENT=\"NOINDEX, NOFOLLOW\">";
echo "<META HTTP-EQUIV=\"Expires\" Content=\"Mon, 28 Mar 1999 00:00:01 GMT\">";
include("top.php");
$err2="Не выбрано ни одной Резюме!<br>";
$error = "";
$maxThread=$maxtext;
...
$n = ereg_replace("\?.*$","",$n);
echo "<center><p><strong><big>Администрирование - Резюме дня</strong></big>";
$srname=$_GET['srname'];
echo ("
...
if ($totalThread == 0){echo "<b>Не установлены Резюме дня</b><br><br>";}
if ($totalThread != 0){
echo ("
...
<tr bgcolor=$maincolor><td colspan=4><b>Резюме дня</b></td></tr>
...
echo "</table></td></tr></table><br><input type=submit value='Убрать Резюме дня' name=delete><br><br>";
}
echo "Выберите Резюме, которые сделать резюме дня (показывать на первой странице):<br><br>";
unset($result);
...
if ($totalThread == 0){echo "<b>Нет резюме</b><br>";}
if ($totalThread != 0){
echo ("
<table border=0 width=95% bgcolor=$bordercolor cellspacing=0 cellpadding=0><tr><td><table border=0 cellspacing=1 cellpadding=4 width=100% class=tbl1>
<tr bgcolor=$maincolor><td><img src=\"picture/basket.gif\" alt=\"Выбрать\" border=0></td><td>#</td><td>Должность</td><td>Зарплата</td></tr>
...
echo "<center><b>Изменения сохранены!</b><br><a href=admindor.php>Вернуться</a><p><br><br><br><br>";
}
