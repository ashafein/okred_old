...
dwr.name AS region,
dwc.name AS city,
CURRENT_DATE, (YEAR(CURRENT_DATE)-YEAR(res.birth)) - (RIGHT(CURRENT_DATE,5) < RIGHT(res.birth,5)) AS age, DATE_FORMAT(res.date,"%d.%m.%Y") as date
FROM '.$restable.' res
...
WHERE res.id='.$link);
		if (@mysql_num_rows($result) == 0) 
			...
			while ($myrow=@mysql_fetch_array($result)){ //4
				...
				$resultaut = @mysql_query('SELECT u.*,
dwr.name AS region_name,
dwc.name AS city_name,
CURRENT_DATE,(YEAR(CURRENT_DATE)-YEAR(u.birth)) - (RIGHT(CURRENT_DATE,5) < RIGHT(u.birth,5)) AS age 
FROM '.$autortable.' u
LEFT JOIN '.$citytable.' dwr ON u.region = dwr.id 
LEFT JOIN '.$citytable.' dwc ON u.city = dwc.id 
WHERE u.id='.$aid);
				...
				$podrazdel1=$myrow["prof_area_join"];
				$razdel1=$myrow["prof_join"];
				echo ("
...
");
				if ($foto1 != ''){
					...
					if (file_exists($photodir.'s'.$fotourl))
						{$fotourl="s$fotourl";}
					echo "<a href=\"photo.php?link=$ID&f=$photodir$foto1&w=$w\" target=_blank><img src=\"$photodir$fotourl\" height=$smallfotoheight alt=\"Увеличить\" border=0></a>";
				}
				elseif ($foto1 == '')
