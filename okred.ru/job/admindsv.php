...
$line = "Страница: |";
for ($k = 1; $k <= $pages; $k++) {
  if ($k != $page) {$line .= "<a href=\"admindsv.php?page=$k\"> $k </a>|";}
  if ($k == $page) {$line .= " $k |";}
}
...
echo "<form name=delreg method=post action=admindsv.php?del>";
$result = @mysql_query("SELECT * FROM $rasvac order by ID DESC LIMIT $initialMsg, $maxThread");
while($myrow=mysql_fetch_array($result)) 
{
$ID=$myrow["ID"];
$aid=$myrow["aid"];
...
$result = @mysql_query("delete from $rasvac where ID=$delmes[$i]");
}
echo "<center><b>Подписка удалена!</b><br><a href=admindsv.php>Вернуться</a><p><br><br><br><br>";
}
}
} //ok
