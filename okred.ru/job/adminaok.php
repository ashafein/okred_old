...
<?php
include("var.php");
echo "<head><title>Администрирование - Одобрение пользователя: $sitename</title>";
...
if ((!isset($_SESSION['uid']) or $_SESSION['uid'] != $adminpass) and ($main != 'main' or $userconfirm != 'checked'))
{
...
$n = ereg_replace("\?.*$","",$n);
$result = @mysql_query("SELECT * FROM $autortable where statconf='wait' order by date DESC");
...
$line = "Страница: |";
for ($k = 1; $k <= $pages; $k++) {
  if ($k != $page) {$line .= "<a href=\"adminaok.php?srname=$srname&srcateg=$srcateg&page=$k\"> $k </a>|";}
  if ($k == $page) {$line .= " $k |";}
}
echo "<center><p><strong><big>Администрирование - Одобрение пользователя</strong></big><p>";
...
if ($srcateg=='moder') {$srcategv='Менеджер';}
echo ("
<form name=sr method=get action=adminaok.php>
...
if ($srcateg == '' or $srcateg == 'nnn') {$result = @mysql_query("SELECT *,CURRENT_DATE,(YEAR(CURRENT_DATE)-YEAR(birth)) - (RIGHT(CURRENT_DATE,5)<RIGHT(birth,5)) AS age FROM $autortable WHERE statconf='wait' $qwery2 order by date DESC LIMIT $initialMsg, $maxThread");}
if ($srcateg != '' and $srcateg != 'nnn') {$result = @mysql_query("SELECT *,CURRENT_DATE,(YEAR(CURRENT_DATE)-YEAR(birth)) - (RIGHT(CURRENT_DATE,5)<RIGHT(birth,5)) AS age FROM $autortable WHERE statconf='wait' and category = '$srcateg' $qwery2 order by date DESC LIMIT $initialMsg, $maxThread");}
...
if ($ustatus=='wait') {$confline="<font color=red>регистрация не подтверждена</font>";}
if ($ustatus != 'wait') {$confline="регистрация подтверждена";}
...
<tr bgcolor=$altcolor><td  colspan=2><b>$categ</b> $confline</td></tr>
...
if ($prof != '') {echo "<b>Должность</b>: $prof<br>";}
...
if ($category=='agency') {$categ='Агентство';}
if ($category=='user') {$categ='Пользователь';}
if ($category=='moder') {$categ='Менеджер';}
$rabtel=$myrow["rabtel"];
$fio=$myrow["fio"];
$prof=$myrow["prof"];
$moder=$myrow["moder"];
$resultmmm = @mysql_query("SELECT ID,firm FROM $autortable WHERE ID = '$moder'");
while ($myrow4=mysql_fetch_array($resultmmm)) 
{
$firm=$myrow4["firm"];
}
...
<tr bgcolor=$maincolor><td valign=top>
");
if ($rabtel != '') {echo "<b>Тел:</b>: $rabtel<br>";}
echo "<b>Организация:</b> $firm<br>";
if ($prof != '') {echo "<b>Должность</b>: $prof<br>";}
if ($fio != '') {echo "<b>Контактное лицо</b>: $fio<br>";}
...
echo ("
<center><input type=submit value='Одобрить отмеченные' name=conf>&nbsp;<input type=submit value='Удалить отмеченные' name=submit><br><br>
</form>
");
}
if ($_SERVER[QUERY_STRING] == "del"){
...
echo "<center><b>Пользователь удален!</b><br><a href=adminaok.php>Вернуться</a><p><br><br><br><br>";
} //del

if (isset($_POST['conf']))
{ // conf
for ($i=0;$i<count($delmes);$i++){
$result=@mysql_query("update $autortable set statconf='ok' WHERE ID=$delmes[$i]");
}
echo "<center><b>Пользователи одобрены!</b><br><a href=adminaok.php>Вернуться</a><p><br><br><br><br>";
} //conf
