...
LEFT JOIN '.$autortable.' u ON vac.aid = u.id
WHERE vac.id='.$texid);
		...
		if ($_SERVER[QUERY_STRING] == "change" and isset($_POST['save'])){ //6
			...
			if (strlen($profecy) > $maxprofecy)
				$error .= "$err1";
			if (strlen($agemin) > $maxmage or strlen($agemax) > $maxmage)
				$error .= "$err2";
			if (strlen($zp) > $maxzp)
				$error .= "$err3";
			if (strlen($stage) > $maxstage)
				$error .= "$err4";
			if (strlen($treb) > $maxcomment)
				$error .= "$err5";
			if (strlen($obyaz) > $maxcomment)
				$error .= "$err6";
			if (strlen($uslov) > $maxcomment)
				$error .= "$err7";
			if (strlen($firm) > $maxfirm)
				$error .= "$err8";
			if (strlen($citynew) > $maxcity)
				$error .= "$err9";
			if (strlen($adress) > $maxadress)
				$error .= "$err10";
...
");
				...
				while($prof_area_row=mysql_fetch_array($prof_area_query)) {
					echo "<option";
					if($prof_area_row["id"] == $razdel) echo " selected";
...
");
				if ($razdel != ''){
					$prof_query = @mysql_query("SELECT * FROM $podcattable WHERE dict_prof_area_id=$razdel order by name");
...
");
						while($prof_row=mysql_fetch_array($prof_query)) {
							echo "<option";
							if($prof_row["id"] == $podrazdel) echo " selected";
...
<select name=region size=1 onChange=location.href=location.pathname+\"?texid=$texid&razdel=$razdel&podrazdel=$podrazdel&region=\"+value+\"\";>
...
");
			...
			while($myrow=mysql_fetch_array($result2)) {
				...
				if($myrow['id'] == $region)
					echo " selected";
				echo ' value="'.$myrow['id'].'">'.$myrow['name'].'</option>';
			}
			echo ("
</select></td></tr>
");
			if ($region != ''){ // регион
				$result1 = @mysql_query("SELECT id, name FROM $citytable WHERE region_ref = $region AND NOT ISNULL(city_ref) ORDER BY name");
				if (@mysql_num_rows($result1) != 0){ // есть регион
					echo ("
...
<option></option>");
			for($i=0; $i<count($edu_arr); $i++){
				echo "<option";
				if($edu_arr[$i] == $edu) echo " selected";
