...
if ($r=='' and $c=='')
	echo "<title>Вакансии : $sitename</title>";
if ($r != ''){
	$resultadd2 = @mysql_query("SELECT id, name FROM $podcattable WHERE id='$c'");
	while($myrow=mysql_fetch_array($resultadd2)){
		$titpodrazdel=$myrow["podrazdel"];
		$titpodrazdel=" : $titpodrazdel";
	}
	$resultadd1 = @mysql_query("SELECT id, name FROM $cattable WHERE id='$r'");
...
");
	...
	while($myrow=mysql_fetch_array($result)){
		...
		if (@mysql_num_rows($resultmdf1) != 0)
			echo "<tr bgcolor=$maincolor><td><a href=\"listvac.php?r=$mid\">$razdel</a></td><td width=40>$totmf1</td></tr>";
		if (@mysql_num_rows($resultmdf1) == 0)
			echo "<tr bgcolor=$maincolor><td><a href=\"listvac.php?r=$mid&c=0\">$razdel</a></td><td width=40>$totmf1</td></tr>";
	}
	echo "</table></td></tr></table>";
} //norazdel
elseif (isset($r) and $r != ''){ // razdel
	if (!isset($c) or $c == ''){ // no podrazdel
...
");
	...
	if (isset($c) and $c != ''){ // list
		...
		if (!isset($maxThread) or $maxThread=='')
			$maxThread = 10;
		if(!isset($page))
			$page = 1;
		...
		for ($k = 1; $k <= $pages; $k++){
			if ($k != $page)
				$line .= "<li><a href=\"listvac.php?r=$r&c=$c&maxThread=$maxThread&ord=$ord&page=$k\"> $k </a></li>";
			if ($k == $page)
				$line .= "<li>$k </li>";
		}
		$line .= "</ul>";
		...
		elseif ($totaltexts != 0){ //2
			echo "<p class=\"path\"> Сортировать  <a href=\"listvac.php?r=$r&c=$c&maxThread=$maxThread&ord=date\">по дате</a> <a href=\"listvac.php?r=$r&c=$c&maxThread=$maxThread&ord=zp\">по зарплате</a></p>";
			if (!isset($ord) or $ord == "")
				$qweryord='date DESC';
			...
			while ($myrow=@mysql_fetch_array($result)){ //4
				...
				$bold=$myrow["bold"];
				$boldstl='';
				if ($bold != '0000-00-00 00:00:00')
					$boldstl="style=\"font-weight: bold;\"";
