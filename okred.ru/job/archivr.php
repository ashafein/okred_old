...
<?php
include("var.php");
echo"<title>Архив резюме : $sitename</title>";
include("top.php");
?>
<h3><center><strong>Архив резюме</strong></center></h3>
<?php
$sid=$_SESSION['sid'];
$id=$_SESSION['sid'];
...
{ //3
$result = @mysql_query("SELECT *,CURRENT_DATE,(YEAR(CURRENT_DATE)-YEAR(birth)) - (RIGHT(CURRENT_DATE,5)<RIGHT(birth,5)) AS age,DATE_FORMAT(date,'%d.%m.%Y') as date,DATE_FORMAT((date + INTERVAL 86400*period SECOND),'%d.%m.%Y') as expir,DATE_FORMAT((top + INTERVAL 86400*$periodfortop SECOND),'%d.%m.%Y') as topq,DATE_FORMAT((bold + INTERVAL 86400*$periodforbold SECOND),'%d.%m.%Y') as boldq FROM $restable WHERE aid = '$id' and status='archive' order by ID DESC");
$totaltexts=@mysql_num_rows($result);
if ($totaltexts == 0) {
echo "<p align=center class=tbl1>Нет объявлений в архиве!<br><br></p>";
}
else
{ //2
...
echo "<p align=center class=tbl1>Всего резюме а архиве: <b>$totaltexts</b></p>";
echo ("
<div align=center><form name=deltext method=post action=archivr.php?delete>
...
$bold=$myrow["bold"];
$topq=$myrow["topq"];
$topql="<br><font color=green>Стоимость поднятия резюме $paytopresume $valutet.</font>";
if ($top != '0000-00-00 00:00:00') {$topql="<br><b>вверху до $topq</b>";}
$boldq=$myrow["boldq"];
$boldql="<br><font color=blue>Стоимость выделения резюме $payboldresume $valutet.</font>";
...
<td valign=top>$expir<br><a href=lres.php?link=$tID target=_blank><small>Подробнее...</small></a></td>
...
if ($pay < $totpricetop)	{$error .= "На вашем счете недостаточно средств для поднятия выбранных резюме!<br>У вас - $pay $valutet. Требуется для поднятия $totob резюме - $totpricetop $valutet.<br>";}
}
// проверка средств на счете
...
if ($pay < $totpricebold)	{$error .= "На вашем счете недостаточно средств для выделения выбранных резюме!<br>У вас - $pay $valute. Требуется для выделения $totob резюме - $totpricebold $valutet.<br>";}
}
// проверка средств на счете
...
echo "<p align=center class=tbl1><h3>Выбранные резюме продлены (убраны их архива)!</h3><a href=autor.php>Вернуться в авторский раздел</a></p><br><br>";
} // long

if (isset($_POST['arch']))
{ // arch
for ($i=0;$i<count($delmes);$i++){
$result=@mysql_query("update $restable set archivedate=now(),status='archive' where ID=$delmes[$i]");
}
echo "<p align=center class=tbl1><h3>Выбранные резюме перемещены в архив!</h3><a href=autor.php>Вернуться в авторский раздел</a></p><br><br>";
} // arch
