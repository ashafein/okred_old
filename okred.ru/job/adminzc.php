...
<?php
include("var.php");
echo "<head><title>Администрирование - Правка учреждений : $sitename</title>";
include("top.php");
echo "<center><h3>Правка учреждений</h3>";
...
{
echo "<center><b>Позиция не определена. Вернитесь на страницу <a href=adminzd.php>удаления!</a></b><br><br>";
}
else
{ // ok
if ($_SERVER[QUERY_STRING] != "add")
{ //2
$result = @mysql_query("SELECT * FROM $zavtable WHERE ID = '$id'");
while ($myrow=mysql_fetch_array($result)) {
$name=$myrow["name"];
...
$sql="update $zavtable SET category='$category',name='$name',comment='$comment',email='$email',telephone='$telephone',adress='$adress',url='$url',foto1='$source_name1',foto2='$source_name2',foto3='$source_name3',foto4='$source_name4',foto5='$source_name5' WHERE ID='$id'";
...
<form method=post ENCTYPE=\"multipart/form-data\" action=\"adminzc.php?add\" name=formata>
...
echo "<center><a href=adminzd.php>Вернуться на страницу удаления</a><br><br><a href=admin.php>На общую страницу администрирования</a><br><br>";
}
else {
echo "<b>Изменения сохранены!</b><br><br><a href=adminzd.php>Вернуться на страницу удаления</a><br><br><a href=admin.php>На общую страницу администрирования</a><br><br>";
}
} // ok
include("down.php");
?>