...
include("var.php");
$maxThread = 20;
echo "<title>Администрирование - Одобрение ссылок: $sitename</title>";
...
{ // ok
echo "<center><p><strong><big>Администрирование - Одобрение ссылок</strong></big>";
echo "<form name=delreg method=post action=adminmpo.php?confirm>";
if ($_SERVER[QUERY_STRING] != "confirm") 
{
$result = @mysql_query("SELECT * FROM $mainpagetable WHERE status='wait' order by date DESC");
$totaltexts=@mysql_num_rows($result);
if ($totaltexts == 0) {echo "<center><b>Нет ссылок для одобрения!</b>";}
else
{ //2
echo "<center>Для одобрения ссылок пометьте их галочкой и нажмите кнопку \"Одобрить\"<br>Всего для одобрения: <b>$totaltexts</b><br><br>";
echo ("
...
echo ("
<br><center><input type=submit value='Одобрить отмеченные' name=conf>&nbsp;<input type=submit name=delete value='Удалить отмеченные'><br><br>
");
} //2
echo "</form>";
}
...
$result=@mysql_query("update $mainpagetable SET status='ok' WHERE ID=$confmes[$i]");
}
echo "<br><br><h3 align=center>Выбранные ссылки одобрены!</h3><br><br>";
}
