...
if ($r=='' and $c=='') {echo "<title>Вакансии : $sitename</title>";}
...
echo "<title>Вакансии : $titrazdel $titpodrazdel $sitename</title>";
}
...
if (!isset($r) or $r=='')
{ //norazdel
$result = @mysql_query("SELECT * FROM $vactable WHERE status='ok' $qwery1");
$totalvac=@mysql_num_rows($result);
unset($result);
echo ("
...
<tr bgcolor=$altcolor><td><a href=listvac.php><b>Вакансии</b></a></td><td width=40>$totalvac</td></tr>
");
...
$mid=$myrow["ID"];
$razdel=$myrow["razdel"];
$resultmf1 = @mysql_query("SELECT * FROM $vactable WHERE razdel='$mid' and status='ok' $qwery1");
$totmf1=@mysql_num_rows($resultmf1);
$resultmdf1 = @mysql_query("SELECT * FROM $catable WHERE razdel='$razdel' and podrazdel != ''");
if (@mysql_num_rows($resultmdf1) != 0) {echo "<tr bgcolor=$maincolor><td><a href=\"listvac.php?r=$mid\">$razdel</a></td><td width=40>$totmf1</td></tr>";}
if (@mysql_num_rows($resultmdf1) == 0) {echo "<tr bgcolor=$maincolor><td><a href=\"listvac.php?r=$mid&c=0\">$razdel</a></td><td width=40>$totmf1</td></tr>";}
}
...
$result = @mysql_query("SELECT * FROM $vactable WHERE status='ok' and razdel=$r $qwery1");
$totalvac=@mysql_num_rows($result);
echo ("
<table border=0 width=100% bgcolor=$bordercolor cellspacing=0 cellpadding=0><tr><td><table border=0 cellspacing=1 cellpadding=4 width=100% class=tbl1>
<tr bgcolor=$altcolor><td><a href=listvac.php?r=$r><b>$razdel</b></a></td><td width=40>$totalvac</td></tr>
");
while($myrow=mysql_fetch_array($resultmdf1)) {
$moid=$myrow["ID"];
$podrazdel=$myrow["podrazdel"];
$resultmdf2 = @mysql_query("SELECT * FROM $vactable WHERE podrazdel='$moid' and status='ok' $qwery1");
$totalmdf2=@mysql_num_rows($resultmdf2);
echo "<tr bgcolor=$maincolor><td><a href=\"listvac.php?r=$r&c=$moid\">$podrazdel</a></td><td>$totalmdf2</td>";
}
echo "</table></td></tr></table>";
}
} // no podrazdel
...
if (isset($c) and $c != '')
{ // list
$result = @mysql_query("SELECT * FROM $vactable WHERE razdel='$r' and podrazdel='$c' and status='ok' $qwery1");
...
$line = "<ul>";
for ($k = 1; $k <= $pages; $k++) {
  if ($k != $page) {$line .= "<li><a href=\"listvac.php?r=$r&c=$c&maxThread=$maxThread&ord=$ord&page=$k\"> $k </a></li>";}
...
$resultmdf1 = @mysql_query("SELECT * FROM $catable WHERE razdel='$razdel' and podrazdel != ''");
//if (@mysql_num_rows($resultmdf1) != 0) {echo "<table border=0 width=100% bgcolor=$bordercolor cellspacing=0 cellpadding=0 align=center><tr><td><table width=100% border=0 cellpadding=1 cellspacing=1 class=tbl1><tr bgcolor=$altcolor><td align=left><b><a href=listvac.php>Вакансии</a> : <a href=listvac.php?r=$r>$razdel</a> : <a href=listvac.php?r=$r&c=$c>$podrazdel</a></b></td></tr></table></td></tr></table><br>";}
//if (@mysql_num_rows($resultmdf1) == 0) {echo "<table border=0 width=100% bgcolor=$bordercolor cellspacing=0 cellpadding=0 align=center><tr><td><table width=100% border=0 cellpadding=1 cellspacing=1 class=tbl1><tr bgcolor=$altcolor><td align=left><b><a href=listvac.php>Вакансии</a> : <a href=listvac.php?r=$r&c=0>$razdel</a></b></td></tr></table></td></tr></table><br>";}
if ($totaltexts == 0) {
...
echo ("
<p class=\"path\"> Сортировать  <a href=\"listvac.php?r=$r&c=$c&maxThread=$maxThread&ord=date\">по дате</a> <a href=\"listvac.php?r=$r&c=$c&maxThread=$maxThread&ord=zp\">по зарплате</a></p>
");

if (!isset($ord) or $ord == "") {$qweryord='date DESC';}
if (isset($ord) and $ord != "") {$qweryord=$ord;}
$result = @mysql_query("SELECT *,DATE_FORMAT(date,'%d.%m.%Y') as dateq FROM $vactable WHERE razdel='$r' and podrazdel='$c' and status='ok' $qwery1 order by top DESC,$qweryord LIMIT $initialMsg, $maxThread");
while ($myrow=@mysql_fetch_array($result)) 
{ //4
$ID=$myrow["ID"];
...
<a href=\"rab.php?id=$aid\">$afirm</a>
    ...
    <td>$date</td>
    <td><a href=\"listvac.php?r=$r&c=$c&page=$page&maxThread=$maxThread&$al=$ID\">$sst</a></td>
  </tr>
</table>
");
...
$basketurl="listvac.php?r=$r&c=$c&page=$page&maxThread=$maxThread";
include("down.php");
?>