...
<?php
include("var.php");
echo "<head><title>Администрирование - Удаление/правка новостей: $sitename</title>";
include("top.php");
echo "<center><h3>Удаление новостей</h3>";
...
$result = @mysql_query("SELECT * FROM $afishatable WHERE ID != 0 $qwery2");
...
$line = "Страница: |";
for ($k = 1; $k <= $pages; $k++) {
  if ($k != $page) {$line .= "<a href=\"adminnd.php?srname=$srname&page=$k\"> $k </a>|";}
  if ($k == $page) {$line .= " $k |";}
}
$result = @mysql_query("SELECT * FROM $afishatable WHERE ID != 0 $qwery2 order by date DESC LIMIT $initialMsg, $maxThread");
$totaltexts=@mysql_num_rows($result);
echo ("
<form name=sr method=get action=adminnd.php>
Строка: <input type=text name=srname size=10>&nbsp;<input type=submit name=submit value='Найти'></form><br><br>
");
echo "<form name=delete_form method=post action=adminnd.php?del>";
if ($totaltexts == 0) {echo "<center><b>Новость не найдена!</b><br><br><a href=admin.php>На общую страницу администрирования</a>";}
else
{ //2
echo "<center>Для удаления позиций пометьте их галочкой и нажмите кнопку \"Удалить\"<br>Всего новостей найдено: <b>$totalThread</b>&nbsp;Показано: <b>$totaltexts</b><br><br>";
echo ("
...
<tr bgcolor=$altcolor><td><input type=checkbox name=allmess value='' onClick=\"SelectAll()\"></td><td><b>Правка</b></td><td><b>№</b></td><td><b>Заголовок</b></td><td><b>Описание</b></td><td><b>Подробности</b></td><td><b>Дата</b></td>
");
echo "</tr>";
while ($myrow=mysql_fetch_array($result)) 
{ //4
...
<td><a href=adminnc.php?id=$ID>Правка</a></td>
<td>$ID</td>
<td>
");
...
Дата: $datum<br>
");
if ($time != '') {echo "Время: $time<br>";}
if ($datumend != '0000-00-00') {echo "До: $datumend<br>";}
echo ("
$preview</td>
<td>$detail</td>
<td>$date</td>
...
$res1 = @mysql_query("SELECT ID,foto1,foto2,foto3,foto4,foto5 FROM $afishatable WHERE ID=$delmes[$i]");
...
$result=@mysql_query("delete from $afishatable WHERE ID=$delmes[$i]");
unset($result);
}
echo "<center><b>Выбранные новости удалены!</b><br><br><a href=adminnd.php>Вернуться на страницу удаления</a><br><br><a href=admin.php>На общую страницу администрирования</a><br><br><br><br>";
} //del in
}
} //del
}
